 ...
 *
 * Idempotency: the scan condition itself (`sr.id IS NULL OR sr.status =
 * 'pending' OR sr.pre_shift_pdf_generated_at < s.updated_at`) is what makes
 * a candidate stop matching once handled — no separate job-id/lock needed
 * beyond the existing `pg_advisory_xact_lock` serializing concurrent ticks.
 */
...
export async function runShiftReportSchedulerCycle(
  ...
  for (const candidate of candidates) {
    const prepared = await tenantContext.runInTenantContext(
      ...
      async (manager) => {
        ...
        return {
          shift,
          html,
          recipientEmails: managerRows.map((r: { email: string }) => r.email) as string[],
          venueName,
        };
      },
    );
    ...
    await tenantContext.runInTenantContext(
      { organisationId: candidate.organisation_id, workspaceId: candidate.workspace_id, userId: '', role: '' },
      async (manager) => {
        let report = await manager.findOne(ShiftReport, { where: { shiftId: candidate.shift_id } });
        if (!report) {
        ...
        report.preShiftPdfSentAt = prepared.recipientEmails.length > 0 ? new Date() : report.preShiftPdfSentAt;
        await manager.save(ShiftReport, report);
      },
    );
