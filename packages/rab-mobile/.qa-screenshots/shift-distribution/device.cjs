...
(async()=>{
 ...
 if(action==='login'){
  ...
  tap(fields[0]);run('shell','input','text',q.email);tap(fields[1]);run('shell','input','text',q.password);run('shell','input','keyevent','4');tap(nodes().find(n=>n['content-desc']==='Log in'));await sleep(2000);
 } else if(action==='tap'){tap(nodes().find(n=>n['content-desc']===arg||n.text===arg));await sleep(700);}
