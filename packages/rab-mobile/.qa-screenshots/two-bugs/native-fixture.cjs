...
async function main() {
  ...
  try {
    ...
    if(process.argv[2]==='cleanup'){
      ...
      const cleanupToken=await login(session.im); for(const record of [session.shift,session.extraUpcoming,session.upcoming].filter(Boolean)) await api(cleanupToken,'/shifts/'+record.id+'/cancel',{reason:'Disposable navigation/card UI QA completed'});
      const cleanup=await tenant.runInTenantContext(context(),async m=>{
        ...
        return {organisationId:session.org.id,workspaceId:session.im.workspaceId,users,unrevokedRefreshTokens:tokens[0].count,shifts,retained:'QA organisation/workspace, venue, profiles, assignments, completed attendance, reports and immutable audit history retained. No hard deletion or RLS changes.'};
      });
    ...
    if(process.argv[2]==='setup'){
      ...
      const s=proof.shift;
      session.qr=qr.sign({id:s.id,venueId:s.venue_id,qrVersion:s.qr_version,startsAt:new Date(s.starts_at),endsAt:new Date(s.ends_at)});
      const payload=signer.verify(session.qr);
      if(payload.shiftId!==s.id||payload.venueId!==s.venue_id||payload.ver!==s.qr_version)throw Error('Fresh QR proof failed');
      await tenant.runInTenantContext(context(),m=>qr.validateQr(m,s.id,session.qr));
      session.qrSha256=crypto.createHash('sha256').update(session.qr).digest('hex');save();
