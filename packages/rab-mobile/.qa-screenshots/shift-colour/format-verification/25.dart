...
void main() {
  ...
  test('29 days since last full auth -> still biometric-eligible', () async {
    ...
    secureStore['rab.biometric.lastFullAuthenticationAt'] = now
  ...
  test(
    '$biometricFullReauthDays days since last full auth -> forces password, no biometric offer',
    () async {
      final now = DateTime.now().toUtc();
      secureStore['rab.accessToken'] = 'access';
      secureStore['rab.refreshToken'] = 'refresh';
      secureStore['rab.biometric.enabledUserId'] = 'user-1';
      secureStore['rab.biometric.lastFullAuthenticationAt'] = now
          .subtract(const Duration(days: biometricFullReauthDays))
          .toIso8601String();
      stubSecureStorageChannel(secureStore);
  ...
  test(
    'password login at the reauth boundary resets the timestamp without touching enrollment',
    () async {
      final now = DateTime.now().toUtc();
      secureStore['rab.accessToken'] = 'access';
      secureStore['rab.refreshToken'] = 'refresh';
      secureStore['rab.biometric.enabledUserId'] = 'user-1';
      secureStore['rab.biometric.lastFullAuthenticationAt'] = now
          .subtract(const Duration(days: biometricFullReauthDays + 1))
          .toIso8601String();
      stubSecureStorageChannel(secureStore);
      ...
      expect(
        secureStore['rab.biometric.enabledUserId'],
        'user-1',
      ); // untouched — no re-enrollment needed
      final resetTimestamp = DateTime.parse(
