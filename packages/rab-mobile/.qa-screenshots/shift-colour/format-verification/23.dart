...
void main() {
  group('biometricLabel', () {
    ...
    test('iOS + fingerprint -> Touch ID', () {
      expect(
        biometricLabel([RabBiometricType.fingerprint], isIOS: true),
        'Touch ID',
      );
    });
    ...
    test('Android + face -> Face Unlock, never "Face ID"', () {
      expect(
        biometricLabel([RabBiometricType.face], isIOS: false),
        'Face Unlock',
      );
    });
    ...
    test('Android + fingerprint -> Fingerprint', () {
      expect(
        biometricLabel([RabBiometricType.fingerprint], isIOS: false),
        'Fingerprint',
      );
    });

    test(
      'no enrolled types -> generic Biometric Login (caller decides whether to show anything at all)',
      () {
    ...
    test(
      'multiple enrolled types -> generic Biometric Login, not a guess at which one wins',
      () {
        expect(
          biometricLabel([
            RabBiometricType.face,
            RabBiometricType.fingerprint,
          ], isIOS: true),
          'Biometric Login',
        );
        expect(
    ...
    test(
      'an unclassified type (Android strong/weak class) -> generic Biometric Login',
      () {
        expect(
          biometricLabel([RabBiometricType.other], isIOS: false),
          'Biometric Login',
        );
      },
    );
  ...
  group('biometricIcon', () {
    ...
    test(
      'unclassified/none/multiple -> the generic fingerprint glyph, never blank',
      () {
        expect(biometricIcon([RabBiometricType.other]), Icons.fingerprint);
        expect(biometricIcon([]), Icons.fingerprint);
        expect(
          biometricIcon([RabBiometricType.face, RabBiometricType.fingerprint]),
          Icons.fingerprint,
        );
      },
    );
  ...
  group('primaryBiometricType', () {
    test('exactly one enrolled type is the primary type', () {
      expect(
        primaryBiometricType([RabBiometricType.face]),
        RabBiometricType.face,
      );
    });
