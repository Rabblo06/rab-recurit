...
void main() {
  ...
  test('successful login authenticates with the correct user', () async {
    final mockClient = MockClient((request) async {
      ...
      if (request.url.path.endsWith('/auth/me')) {
        return http.Response(
          jsonEncode(
            currentUserJson(id: 'user-1', email: 'alice@example.test'),
          ),
          200,
        );
      }
  ...
  test(
    'failed login (401) leaves the session unauthenticated and throws ApiException',
    () async {
      final mockClient = MockClient((request) async {
        if (request.url.path.endsWith('/auth/login')) {
          return http.Response(
  ...
  test(
    'session restore with a stored token resolves to authenticated without a fresh login call',
    () async {
  ...
  test(
    'session restore where /auth/me rejects the token ends unauthenticated and clears stored tokens',
    () async {
  ...
  test(
    'logout revokes the refresh-token family server-side and clears the local session',
    () async {
      ...
      expect(
        (jsonDecode(logoutRequest!.body) as Map)['refreshToken'],
        'refresh-1',
      );
      expect(secureStore.containsKey('rab.accessToken'), isFalse);
      expect(secureStore.containsKey('rab.refreshToken'), isFalse);
