...
async function main() {
  ...
  try {
    ...
    if(process.argv[2]==='cleanup'){
      ...
      const cleanupToken=await login(session.im); for(const record of (session.colourShifts??[session.shift,session.extraUpcoming,session.upcoming].filter(Boolean))) await api(cleanupToken,'/shifts/'+record.id+'/cancel',{reason:'Disposable navigation/card UI QA completed'});
      const cleanup=await tenant.runInTenantContext(context(),async m=>{
        ...
        return {organisationId:session.org.id,workspaceId:session.im.workspaceId,users,unrevokedRefreshTokens:tokens[0].count,shifts,retained:'QA organisation/workspace, venue, profiles, cancelled shifts, assignments and immutable audit history retained. No attendance actions, hard deletion or RLS changes.'};
      });
    ...
    if(process.argv[2]==='five'){
      ...
      const matrix=session.colourShifts.map(s=>{let h=0;for(const c of s.id)h=(h*31+c.charCodeAt(0))%2147483647;return {shiftId:s.id,startsAt:s.startsAt,colour:['lavender','yellow','peach','mint','blue'][h%5]}});
