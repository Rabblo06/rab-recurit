...
class BiometricSetupSheetContent extends StatefulWidget {
  ...
  @override
  State<BiometricSetupSheetContent> createState() => _BiometricSetupSheetContentState();
}

class _BiometricSetupSheetContentState extends State<BiometricSetupSheetContent> {
  bool _busy = false;
  ...
  Future<void> _respond(bool enable) async {
    setState(() {
      _busy = true;
      _visual = enable ? RabBiometricVisualState.scanning : RabBiometricVisualState.idle;
    });
    final outcome = await context.read<AuthProvider>().completeBiometricSetup(enable: enable);
    if (!mounted) return;
    if (enable) {
      setState(() => _visual = outcome == BiometricOutcome.success ? RabBiometricVisualState.success : RabBiometricVisualState.failure);
    }
      ...
      children: [
        Center(
          child: Container(
            ...
            decoration: BoxDecoration(color: colors.atmosphereDeep, borderRadius: BorderRadius.circular(AppRadius.xl)),
            child: RabBiometricVisual(state: _visual, icon: biometricIcon(_enrolledTypes)),
          ),
        ),
        Column(
        ...
        SizedBox(
          ...
          height: 56,
          child: FilledButton(
            style: FilledButton.styleFrom(backgroundColor: colors.accentStrong, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(AppRadius.full))),
            onPressed: _busy ? null : () => _respond(true),
            child: _busy && _visual == RabBiometricVisualState.scanning
                ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
                : Text('Enable $label', style: text.bodyMobile.copyWith(color: Colors.white, fontWeight: FontWeight.w600)),
          ),
        ),
        Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            SizedBox(
              ...
              child: OutlinedButton(
                style: OutlinedButton.styleFrom(side: BorderSide(color: colors.border, width: 1.5), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(AppRadius.full))),
                onPressed: _busy ? null : () => _respond(false),
                child: Text('Skip for now', style: text.bodyMobile.copyWith(fontWeight: FontWeight.w600)),
              ),
            ),
            const SizedBox(height: AppSpace.s4),
            Center(child: Text('You can turn this on any time in Profile', style: text.label, textAlign: TextAlign.center)),
          ],
        ),
      ],
    );
